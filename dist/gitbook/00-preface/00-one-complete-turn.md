...
> **Status / 状态:** Complete **Harness milestone / Harness 里程碑:** A tiny offline turn and its first behavioural tests.

## 先看全局 / Big picture

我们最终想做的，不是一个只会把文字送给模型、再把答案打印出来的聊天脚本。

我们要做的是一个本地 **智能体运行框架** *Agent Harness*。
...
**会话** *Thread* 是一段可以持续下去的对话。

这里的 Thread 不是操作系统线程。它更像一个项目对话容器：前面的消息和结果可以成为后面回合的上下文。

#### Turn / 回合

**回合** *Turn* 从一次用户输入开始，包含模型和工具随后完成的工作，最后以完成、失败或中断结束。
...
把基础类型包成不同类型，叫 **新类型模式** *Newtype Pattern*。完整语法会在第 5 章再讲。现在只需要知道：相同的数字，不等于相同的业务含义。
...
核心返回数据，外层处理 I/O，这种组织方式常被称为 **函数式核心、命令式外壳** *Functional Core, Imperative Shell*。
...
这正是本书会一直使用的 **测试驱动开发** *Test-Driven Development (TDD)*。
...
这个测试不是直接调用 `render_event`。它启动 Cargo 构建出的真实二进制，再比较标准输出，所以它是 **黑盒测试** *Black-Box Test*。
...
想单独保存或修改本章结束时的完整项目，可以使用 [第 0 章代码包 / Chapter 0 code pack](../_delivery/ch00.md)。下载页记录固定源 commit、校验值和从干净目录开始的验证步骤。
