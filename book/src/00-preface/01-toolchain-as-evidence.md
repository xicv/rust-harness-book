...
<p class="card-label">本章核心 / Core idea</p>
...
**工具链** <span class="term-en">Toolchain</span> 不是只有 `rustc`。
...
**Rust 版本代际** <span class="term-en">Rust Edition</span> 决定一份源码使用哪一代语言规则和惯用法。
...
**最低支持 Rust 版本** <span class="term-en">Minimum Supported Rust Version (MSRV)</span> 表示这个项目承诺至少支持多新的 Rust。
...
**工作空间** <span class="term-en">Workspace</span> 把相关 package 放在一个共同根目录管理。它们可以共享：
...
**Crate** <span class="term-en">Crate</span> 是 Rust 编译器一次处理的编译单元。
...
本项目显式使用 **依赖解析器 3** <span class="term-en">Dependency Resolver 3</span>：
...
**锁文件** <span class="term-en">Lockfile</span> `Cargo.lock` 记录某一次实际解析得到的精确依赖结果。
...
<div class="learning-card">
<p class="card-label">当前证据 / Current evidence</p>
...
[第 1 章代码包 / Chapter 1 code pack](../_delivery/ch01.md)。下载页记录固定源
