...
我们会一边学习 Rust，一边完成一个真实项目：一个可以在本地运行、可以测试、可以解释，也可以逐步验证的 **智能体运行框架** <span class="term-en">Agent Harness</span>。
...
全书会从一个最小离线回合开始，逐章加入类型、所有权、接口、异步、工具、权限、持久化、恢复、协议和评测。最后，我们会得到一个真的可以运行和验证的本地 Harness。
...
学习 Rust 很少是一次读完就全部掌握。Rust 项目在 2026 年发布的学习研究也指出，不同背景的学习者会走不同路线，很多人需要用官方文档、练习、视频、真实项目和编译器反馈多次建立同一个心智模型。
...
**测试驱动开发** <span class="term-en">Test-Driven Development (TDD)</span> 不是为了追求更多测试，也不是先写一段注定失败的代码做仪式。
